...
import { normalizeClosed, normalizeCloseStatus, normalizeSessions } from "./sessions.js";
	...
	fc.record(
		{
			...
			browser: fc.option(fc.record({ browserName: safeArgArb, userDataDir: safeArgArb }), { nil: undefined }),
		},
		{ requiredKeys: [] },
	),
);
	...
	it("always returns count/empty invariants for arbitrary session input", () => {
		fc.assert(
			fc.property(jsonValueArb, (value) => {
				...
				expect(sessions.servers.empty).toBe(sessions.servers.count === 0 ? "no attachable browser servers" : undefined);
				expect(sessions.channelSessions.count).toBe(sessions.channelSessions.rows.length);
				expect(sessions.channelSessions.empty).toBe(sessions.channelSessions.count === 0 ? "no channel sessions" : undefined);
		...
		fc.assert(
			fc.property(
				...
				(servers, channelSessions) => {
					const sessions = normalizeSessions({ browsers: [], servers, channelSessions });
					...
					expect(sessions.channelSessions.rows).toHaveLength(channelSessions.length);
				},
			),
			propertyOptions,
		);
