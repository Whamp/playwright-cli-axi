...
export function commandSuccessModel(
	...
	if (parsed.kind === "json") {
		if (command === "list") {
			...
			return {
				...
				browsers: {
					...
					...(sessions.browsers.empty
						? { empty: sessions.browsers.empty }
						: {}),
				},
		...
		if (isObject(parsed.value))
			return { command, status: "ok", result: pruneJson(parsed.value) };
...
function pruneJson(value: Record<string, unknown>): ToonValue {
	...
	for (const [key, child] of Object.entries(value)) {
		...
		if (
			...
			output[key] = child.map((entry) => simpleValue(entry)) as ToonValue;
