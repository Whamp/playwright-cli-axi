export type ToonValue = null | boolean | number | string | ToonValue[] | { [key: string]: ToonValue } | ToonTable;
...
export function table(fields: string[], rows: Record<string, ToonValue>[]): ToonTable {
...
function encodeRoot(value: ToonValue): string[] {
  if (isPlainObject(value)) {
    return Object.entries(value).flatMap(([key, child]) => encodeNamed(key, child, 0));
  }
...
function encodeNamed(key: string, value: ToonValue, indent: number): string[] {
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value) {
      if (isPlainObject(item)) {
        ...
        lines.push(`${' '.repeat(indent + 2)}- ${firstKey}: ${formatScalar(firstValue as string | number | boolean | null)}`);
        for (const [childKey, childValue] of entries.slice(1)) {
          lines.push(...encodeNamed(childKey, childValue, indent + 4));
        }
      } else {
        lines.push(`${' '.repeat(indent + 2)}- ${formatScalar(item as string | number | boolean | null)}`);
      }
    }
    return lines;
  }
  if (isPlainObject(value)) {
    ...
    return [`${prefix}${key}:`, ...entries.flatMap(([childKey, childValue]) => encodeNamed(childKey, childValue, indent + 2))];
  }
  return [`${prefix}${key}: ${formatScalar(value as string | number | boolean | null)}`];
}
...
function quoteIfNeeded(value: string): string {
  ...
  if (/^(true|false|null|-?\d+(?:\.\d+)?(?:e[+-]?\d+)?)$/i.test(value)) return quote(value);
...
function assertInvariants(output: string): void {
  if (output.includes('\r')) throw new Error('TOON output must use LF line endings');
  if (output.endsWith('\n')) throw new Error('TOON output must not end with a trailing newline');
  for (const line of output.split('\n')) {
    if (/\s$/.test(line)) throw new Error(`TOON output has trailing whitespace: ${line}`);
  }
}
